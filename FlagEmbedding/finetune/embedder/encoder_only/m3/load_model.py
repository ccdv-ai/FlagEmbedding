...
def get_model(model_args: EncoderOnlyEmbedderM3TrainingArguments, output_dir: str, resize: bool, resize_tokens: int):
    """Get the model.
    ...
    """
    cache_folder = os.getenv('HF_HUB_CACHE', None) if model_args.cache_dir is None else model_args.cache_dir
    ...
    colbert_linear = torch.nn.Linear(
        ...
        out_features=model.config.hidden_size if model_args.colbert_dim <= 0 else model_args.colbert_dim
    )
    ...
    colbert_model_path = os.path.join(model_args.model_name_or_path, 'colbert_linear.pt')
    sparse_model_path = os.path.join(model_args.model_name_or_path, 'sparse_linear.pt')
...
def save_merged_model(model_args: EncoderOnlyEmbedderM3TrainingArguments, output_dir: str):
    """
